...
from bots.base import BaseScraper
from news_selectors import AljazeeraSelectors as ajs
...
class AlJazeeraScraper(BaseScraper):
    ...
    def parse(self):
        ...
        while not self.close_spider:
            ...
            self.browser.wait_until_element_is_not_visible(ajs.loading_animation_xpath, timeout=10)

    def accept_cookies(self):
        """Accept cookies to proceed with the website scraping."""
        if self.browser.is_element_visible(ajs.cookie_button_xpath):
            self.browser.wait_and_click_button(ajs.cookie_button_xpath)
    ...
    def browser_get_articles(self):
        ...
        results = self.wait_for_element_and_get_results(ajs.search_results_xpath, max_retries=5)
    ...
    def sort_by_latest(self):
        ...
        self.wait_for_element_and_get_result(ajs.sort_by_xpath, max_retries=5)
        self.browser.select_from_list_by_value(ajs.sort_by_select_xpath, 'date')
    ...
    def click_next_item(self):
        ...
        if self.browser.is_element_visible(ajs.click_next_button_xpath):
            self.browser.click_element_when_clickable(ajs.click_next_button_xpath)
        log.debug("Next button clicked")

    def extract_articles(self, articles):
        """Extract information from each article element."""
        for article in articles:
            title = article.find_element(By.XPATH, ajs.article_title_xpath).text
            description = article.find_element(By.XPATH, ajs.article_description_xpath).text
            ...
            image_url = article.find_element(By.XPATH, ajs.article_image_xpath).get_attribute('src')
